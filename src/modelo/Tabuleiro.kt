...
enum class TabuleiroEvento { VITORIA, DERROTA, TEMPO_ESGOTADO }
...
class Tabuleiro(val qtdeLinhas: Int, val qtdeColunas: Int, private val qtdeMinas: Int) {
    ...
    private fun objetivoAlcancado(): Boolean {
        return campos.flatten().all { it.objetivoAlcancado }
    }
    ...
    private fun iniciarTimer() {
        ...
        timer?.schedule(object : TimerTask() {
            override fun run() {
                callbacks.forEach { it(TabuleiroEvento.TEMPO_ESGOTADO) }
            }
        }, 1 * 120 * 1000)
    }
